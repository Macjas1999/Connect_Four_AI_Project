...
class ConnectFourAI:        
    def __init__(self,  pl_1_savefl, pl_2_savefl):
        ...
        self.model_saved_filename = pl_1_savefl
        self.model_saved_filename = pl_2_savefl

        self.analyzer = AnalyzeLayout()

    def build_model(self):
        model = Sequential()
        model.add(Reshape((6,7,1), input_shape=(42, ))) #(42,)
    ...
    # def reshape_board_to_input(self, board):
    #     input_array = np.array(board).reshape((1, 6, 7, 1))
    #     return input_array    
    ...
    #
    def make_move(self, model, board):
        action_probabilities = model.predict(self.reshape_board_to_input(board))[0]
    ...
    def train_player(self, model, X_train, y_train, sample_weights, epochs=100):
        model.fit(X_train, y_train, sample_weights, epochs=epochs, verbose=0)

    def train_player_v2(self, data_folder_name):
        handler = TrainingDataHandler(data_folder_name)
        for i in handler.list_of_datafiles:
            handler.handler_get_state(i)
            if handler.player == 1:
                self.model_1.fit(self.reshape_board_to_input(handler.data), handler.move, verbose = 0)
            elif handler.player == 2:
                self.model_2.fit(self.reshape_board_to_input(handler.data), handler.move, verbose = 0)
            else:
                raise
                


    # def train_player_1(self, model, X_train, y_train, player, num_samples, epochs=100):
    #     for epoch in range(epochs):
    #         for i in range(num_samples):
    #             state = self.get_next_gamestate(X_train, y_train, player)  # Replace with your function to get the next game state
    ...
    # def train_player_2(self, model, X_train, y_train, player, num_samples, epochs=100):
    #     for epoch in range(epochs):
    #         for i in range(num_samples):
    #             state = self.get_next_gamestate(X_train, y_train, player)  # Replace with your function to get the next game state
    #             action = model.predict(self.reshape_board_to_input(state))
...
if __name__ == "__main__":
    ai = ConnectFourAI('null', 'null')
    #ai.train_player_v2("data")
    ...
    for i in range(0,6):
        ...
        test_board[i][5] = 2
    print(test_board)
    print(ai.get_valid_moves(test_board))
    print(ai.make_move(ai.model_2, test_board))
    print(ai.model_1.predict(ai.reshape_board_to_input(test_board))[0])
    print(ai.model_1.predict(ai.reshape_board_to_input(test_board))[0] * ai.get_valid_moves(test_board))