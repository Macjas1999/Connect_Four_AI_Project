...
class ConnectFourGUI:
    ...
    def _update_cell(self, row, col):
        ...
        if value == 0:
            ...
            # Player 1 - saturated blue circle
            ...
            if owner == '1':
                ...
                self._draw_circle(canvas, 'magenta', 'darkmagenta')
            else:
    ...
    def _end_game(self):
        ...
        if start_again:
            # Ask which player the human wants to be (1 or 2)
            while True:
                try:
                    choice = simpledialog.askinteger('New Game', 'Which player do you want to play as? (1 or 2)')
                    if choice is None:
                        return
                    if choice in (1, 2):
                        break
    ...
    def _start_new_game(self, human_player):
        # Reset board array and state
        self.board.array = [[0 for _ in range(self.board.cols)] for _ in range(self.board.rows)]
        self.board.winning = 0
        self.board.run = True
        self.board.player_one_wins = 0
        self.board.player_two_wins = 0

        # Configure AI player as the opposite of human
        self.board.ai_player = 2 if human_player == 1 else 1
