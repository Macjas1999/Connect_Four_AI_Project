...
class TrainingDataHandler:
    def __init__(self, dir):
        ...
        #self.data = np.array([])
        #self.labels_1 = np.array([])
        #self.labels_2 = np.array([])
    ...
    def extract_data(self):
        ...
        for filename in os.listdir(self.directory):
            if filename.endswith(".csv"):
                ...
                #self.data = np.vstack((self.data, flattened_array))


                self.labels_1.append(self.extract_score_1(filename))
                self.labels_2.append(self.extract_score_2(filename))
                #self.labels_1 = np.vstack((self.labels_1, self.extract_score_1(filename)))
                #self.labels_2 = np.vstack((self.labels_2, self.extract_score_2(filename)))
